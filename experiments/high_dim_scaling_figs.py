...
For each dimension d, lift the close_separable dataset into R^d (signal in a
random 2D subspace, isotropic noise elsewhere), then measure how many Monte
Carlo samples per example a standard SVM needs to match the closed-form SVM-GMU
boundary (angle <= tau). Headline: N*(d). Secondary: fixed-budget angle vs d.
...
def run_highdim_sweep(d_values, n_ladder, seeds, sigma2_noise):
    ...
    for di, d in enumerate(d_values):
        for si, seed in enumerate(seeds):
            ...
            Xl, sul, _ = C.lift_dataset(X, SAMPLE_UNCERTAINTY, d, sigma2_noise, rng)
            w_ref, b_ref = C.fit_gmu(Xl, y, sul)
            eval_cloud = C.mc_eval_cloud(sul, 2000, rng)
    ...
    return {
        ...
        "seeds": np.array(seeds), "sigma2_noise": sigma2_noise,
...
def _cache_path(mode):
    return CACHE_DIR / f"high_dim_{mode}.npz"


def _load_or_run(mode, force, d_values, n_ladder, seeds, sigma2_noise):
    path = _cache_path(mode)
    if path.exists() and not force:
        data = np.load(path, allow_pickle=False)
        print(f"loaded cache {path}")
        return {k: data[k] for k in data.files}
    res = run_highdim_sweep(d_values, n_ladder, seeds, sigma2_noise)
...
def main():
    ...
    res = _load_or_run(mode, args.force, d_values, n_ladder, seeds, SIGMA2_NOISE)

    if args.smoke:
        # Sanity: at d=2 the smallest budget should already be close; angle at
        # the top of the ladder should not increase with d for fixed N.
